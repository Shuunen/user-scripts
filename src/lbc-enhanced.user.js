...
class LBCEnhanced {
  ...
  findKeywords (element = DomElement, html = '', keywords) {
    ...
    var icon = this.addIcon(element, founds.join(', '), 0)
    var width = 300
  ...
  addIconsForPricePerMeter (element = DomElement, price) {
    var icon = this.addIcon(element, `${price} € / m²`, 129, 8)
  ...
  addIconsForClasses (element = DomElement, list = ['?', '?']) {
    ...
    var right = 15
    list.forEach(cls => {
      var icon = this.addIcon(element, cls, top, right)
  ...
  async enhanceListItem (element = DomElement) {
    ...
    var html = await fetch(element.link.href).then(r => r.text())
